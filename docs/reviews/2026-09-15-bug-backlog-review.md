...
| [BUG-010](../bugs/fixed/010-event-time-and-recurrence-boundaries.md) | The boundary/time findings are supported. D5 now requires an event timezone field, defaulting to the team timezone, used consistently for input, display, communications and recurrence. Preserve stored instants during backfill and existing events when the team default changes. |

...
- **Web** — after a successful creation, the form calls a server action to delete the `active_profile_id` cookie before calling `router.push('/dashboard')`. This is necessary for the same reason as iOS: the dashboard layout reads `active_profile_id` from the cookie at render time (line 33), and `router.refresh()` alone does not clear it. If the cookie holds a managed profile ID, the layout will attempt to resolve that profile's membership on the new team, find none, and fall through to `allMemberships[0]` non-deterministically. Explicitly deleting the cookie before the redirect ensures the layout resolves to the own profile on the next render. A small `clearActiveProfile()` server action (deleting the `active_profile_id` cookie) should be added alongside the existing `setActiveTeam` action in `apps/web/src/app/actions/team.ts`, or the cookie deletion can be folded into `setActiveTeam` directly since resetting to own profile is always the correct behaviour when switching to a newly created team.
...
**Chat channel provisioning is part of this contract.** The `teams` INSERT in step 3 fires the `create_team_channel` trigger (`supabase/migrations/20260307000000_team_chat.sql`, line 68), which creates the team-wide `channels` row. No `channel_members` row is seeded by the trigger — nor is one needed. The `channels_select` RLS policy grants access to `type = 'team'` channels via `is_team_member(team_id)`, so any team member (including the new coach) can access the team channel by virtue of their `team_members` row alone. This is not an incidental side effect — it is a guaranteed part of what "create a team" means in this system. The spec calls it out explicitly so that any future changes to the RPC (e.g. a dry-run mode, a bulk-import path, or a test helper that bypasses the trigger) are made with full awareness that chat provisioning would need to be handled separately.
